 ...
 * <p>
 * To link classes with defects, for a bug that was fixed and closed in a
 * version v, we computed which classes were modified during the fix change. We
 * thus measure the defect-proneness of a class c in version v using the
 * following three different ways, respectively: 1. The number of times class c
 * is involved in fixing bugs that were injected in version v, that is, the
 * version where the bugs were found and reported. 2. The number of times class
 * c is involved in fixing bugs that were resolved in version v. 3. The number
 * of times class c is involved in fixing bugs that were alive in version v;
 * that is, the bugs were reported before or in version v and were resolved
 * after or in version v.
 */
