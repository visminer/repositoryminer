...
public class BOvR implements IIndirectCodeMetric {
	...
	// The class name is the key and the list of its methods signature is the value
	...
	public void calculate(AbstractClassDeclaration type, AST ast) {
		...
		List<String> methodsList = new ArrayList<String>(cls.getMethods().size());
	...
	public Map<String, Document> getResult() {
		...
		for (Entry<String, Float> entry : bovr.entrySet()) {
			result.put(entry.getKey(), new Document("metric", CodeMetricId.BOvR.toString()).append("value", entry.getValue()));
		}
