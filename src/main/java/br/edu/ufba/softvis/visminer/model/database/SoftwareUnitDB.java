...
@NamedQueries({
  @NamedQuery(name = "SoftwareUnitDB.findByUid", 
      query = "select sw from SoftwareUnitDB sw where sw.uid = :uid"),
  @NamedQuery(name = "SoftwareUnitDB.findByRepository", 
  query = "select swu from SoftwareUnitDB swu inner join swu.commits c where swu.repository.id = "
      + ":repositoryId and c.id = :commitId") })
public class SoftwareUnitDB implements Serializable {
  private static final long serialVersionUID = 1L;

  @Id
  @SequenceGenerator(name = "SOFTWARE_UNIT_ID_GENERATOR",
  sequenceName = "SOFTWARE_UNIT_SEQ")
  @GeneratedValue(strategy = GenerationType.SEQUENCE,
  generator = "SOFTWARE_UNIT_ID_GENERATOR")
  ...
  public static List<SoftwareUnit> toBusiness(
    ...
    for (SoftwareUnitDB su : softwareUnits) {
      ...
      bizzSu.setChildren(new ArrayList<SoftwareUnit>());
      bizzSu.setMetricValues(new HashMap<MetricUid, String>());
