...
@NamedQueries({
  @NamedQuery(name = "CommitterDB.findByEmail", 
      query = "select c from CommitterDB c where c.email = :email"),
  @NamedQuery(name = "CommitterDB.findByRepository", query = "select c from RepositoryDB r inner"
      + " join r.committers c inner join c.commits c2 where r.id = :repositoryId group by "
      + "c.id order by min(c2.date) asc") })
public class CommitterDB implements Serializable {
  ...
  @ManyToMany
  @JoinTable(name = "committer_contribute_repository", joinColumns = { 
      @JoinColumn(name = "committer_id", nullable = false) }, inverseJoinColumns = { 
