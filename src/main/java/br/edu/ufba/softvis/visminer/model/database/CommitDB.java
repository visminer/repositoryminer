...
@Table(name = "commit")
@NamedQueries({ 
  @NamedQuery(name = "CommitDB.findByTree", 
    query = "select c from CommitDB c join c.trees t where t.id = :id order by c.date asc"),
  @NamedQuery(name = "CommitDB.findByRepository", 
    query = "select distinct(c)from CommitDB c join c.trees t where t.repository.uid = :uid order"
        + " by c.date asc"),
  @NamedQuery(name = "CommitDB.findNotRefIssue", 
    query = "select distinct(c) from TreeDB t join t.commits c left join c.commitReferenceIssues"
        + " cri where t.repository.id = :repoId and cri.id.commitId is null"),
  @NamedQuery(name = "CommitDB.findIdsUntilDate", 
    query = "select distinct(c.id) from CommitDB c join c.trees t where t.repository.id ="
        + " :repositoryId and c.date <= :until order by c.date asc")
})
public class CommitDB implements Serializable{
  ...
  @ManyToMany
  @JoinTable(name = "software_unit_x_commit", joinColumns = { @JoinColumn(name = "commit_id", 
  nullable = false) },
  ...
  public Commit toBusiness() {
    ...
    commit.setCommands(new ArrayList<IssueCommand>());
    commit.setCommands(new ArrayList<IssueCommand>());
