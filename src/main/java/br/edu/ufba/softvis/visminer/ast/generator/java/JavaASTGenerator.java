...
public class JavaASTGenerator implements IASTGenerator{
  ...
  public AST generate(String filePath, String source, String[] sourceFolders){
    ...
    /* 
     * bind resolving does not work well when is not possible to
     * access the other piece of code.
     */
    try{
    ...
    for(int i = 0; i < root.imports().size(); i++){
      org.eclipse.jdt.core.dom.ImportDeclaration importAux = 
          (org.eclipse.jdt.core.dom.ImportDeclaration) root.imports().get(i);
    ...
    List<br.edu.ufba.softvis.visminer.ast.TypeDeclaration> typesDecls = new 
  ...
  private static br.edu.ufba.softvis.visminer.ast.TypeDeclaration processType(TypeDeclaration 
      type, String pkgName){
  ...
  private static MethodDeclaration processMethod(org.eclipse.jdt.core.dom.MethodDeclaration 
      methodDecl){
    ...
    for(SingleVariableDeclaration var : (List<SingleVariableDeclaration>) 
  ...
  private static br.edu.ufba.softvis.visminer.ast.EnumDeclaration processEnum(EnumDeclaration 
      enumType, String pkgName){

    br.edu.ufba.softvis.visminer.ast.EnumDeclaration enumDecl = new 
        br.edu.ufba.softvis.visminer.ast.EnumDeclaration();
    ...
    for(Object elem : enumType.enumConstants()){
      ...
      org.eclipse.jdt.core.dom.EnumConstantDeclaration constEnum = 
          (org.eclipse.jdt.core.dom.EnumConstantDeclaration) elem;

      EnumConstantDeclaration constDecl = new EnumConstantDeclaration();
      constDecl.setName(enumDecl.getName() + "." + constEnum.getName().
