...
public class JavaASTGenerator implements IASTGenerator {
	...
	private br.edu.ufba.softvis.visminer.ast.TypeDeclaration processType(
			TypeDeclaration type, String pkgName) {
		...
		clsDecl.setName(clsDecl.getName()
				+ type.getName().getFullyQualifiedName());

		if (type.getFields().length > 0) {
			List<FieldDeclaration> fields = new ArrayList<FieldDeclaration>();
			for (org.eclipse.jdt.core.dom.FieldDeclaration field : type
					.getFields()) {
	...
	private MethodDeclaration processMethod(
		...
		ModifierKeyword modifier = ModifierKeyword.fromFlagValue(methodDecl
				.getModifiers());
		if (modifier != null)
	...
	@SuppressWarnings("unchecked")
	private FieldDeclaration processField(
	...
	private br.edu.ufba.softvis.visminer.ast.EnumDeclaration processEnum(
			EnumDeclaration enumType, String pkgName) {
		...
		enumDecl.setName(enumDecl.getName()
				+ enumType.getName().getFullyQualifiedName());
		...
		if (enumType.enumConstants().size() > 0) {
			...
			for (Object elem : enumType.enumConstants()) {
				...
				constDecl.setName(enumDecl.getName() + "."
						+ constEnum.getName().getFullyQualifiedName());
